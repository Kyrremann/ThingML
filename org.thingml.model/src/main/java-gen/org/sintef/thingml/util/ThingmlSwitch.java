...
public class ThingmlSwitch<T> extends Switch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			...
			case ThingmlPackage.TRANSITIONS: {
				Transitions transitions = (Transitions)theEObject;
				T result = caseTransitions(transitions);
				if (result == null) result = caseHandler(transitions);
				if (result == null) result = caseAnnotatedElement(transitions);
				if (result == null) result = caseThingMLElement(transitions);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
